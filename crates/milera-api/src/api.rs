...
impl MileraApi {
    ...
    #[cfg_attr(target_family = "wasm", wasm_bindgen)]
    pub async fn login_user(&self, username: &str, password: &str) -> Result<LoginResponse, ApiError> {
        http_post!(
...
#[cfg_attr(target_family = "wasm", wasm_bindgen)]
pub async fn register_user(api: &MileraApi) -> Result<RegistrationResponse, ApiError> {
    Ok(api.register_user("username", "password").await?)
}
