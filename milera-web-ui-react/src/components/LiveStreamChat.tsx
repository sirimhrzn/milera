...
import { getChatMessagesByDiscussion, type ChatMessage } from "@/lib/data"
...
function InitialAvatar({ name, size = "w-6 h-6" }: { name: string; size?: string }) {
  const initials = getInitials(name)

  return (
    <div className={`${size} bg-gray-500 rounded-full flex items-center justify-center flex-shrink-0`}>
...
export function LivestreamChat({ discussionId }: LivestreamChatProps) {
  ...
  useEffect(() => {
    ...
    const randomAuthors = ["LiveViewer", "ChatUser", "Observer", "Commenter", "Participant", "Watcher"]

    const interval = setInterval(() => {
      if (Math.random() > 0.7) {
        // 30% chance every 3 seconds
        const discussionMessages = getRandomMessagesForDiscussion(discussionId)
  ...
  return (
    <div className="w-80 bg-background border-l border-border flex flex-col h-full max-h-[calc(100vh-4rem)]">
      {/* Chat Header */}
      ...
      {/* Messages Container */}
      <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-2 space-y-2 overscroll-contain">
        {messages.map((message) => (
          <div key={message.id} className="flex space-x-2 text-sm">
            <InitialAvatar name={message.author} />
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-1">
                <span className={`font-medium text-xs ${message.author === "You" ? "text-blue-500" : ""}`}>
                  {message.author}
                </span>
                {message.verified && (
              ...
              <p className="text-xs text-foreground break-words leading-relaxed">{message.message}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="p-3 border-t border-border bg-muted/30 flex-shrink-0">
        ...
        <p className="text-xs text-muted-foreground mt-1 text-center">{newMessage.length}/200</p>
      </div>
    </div>
  )
}
