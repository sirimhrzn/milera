...
// Helper functions
export function getPostsByDiscussion(discussionId: string): Post[] {
  return posts.filter((post) => post.discussionId === discussionId)
}

export function getChatMessagesByDiscussion(discussionId: string): ChatMessage[] {
  return chatMessages.filter((message) => message.discussionId === discussionId)
}
