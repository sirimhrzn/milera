      ...
      <div className="space-y-3">
        {options.map((option, index) => (
          <div key={index} className="flex gap-2">
            ...
            />
            {options.length > 2 && (
              <Button variant="ghost" size="sm" onClick={() => removeOption(index)} className="px-2">
                ×
              </Button>
            )}
          </div>
        ...
        {options.length < 4 && (
          <Button variant="ghost" onClick={addOption} className="w-full text-primary hover:bg-primary/10">
            Add choice
          </Button>
        )}
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => onSave({ options: options.filter((o) => o.trim()), duration })}
              disabled={!canSave}
              size="sm"
            >
              Save
